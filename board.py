...
class Move:
    ...
    def __str__(self):
        return f"Move from {self.origin} to {self.target}"
        ...
        if self.default is None:
            ...
            if self.row == 1:
                ...
                if self.column in (0, 7):
                    self.default = Rook(color)
                if self.column in (1, 6):
                    self.default = Knight(color)
                if self.column in (2, 5):
                    self.default = Bishop(color)
                if self.column == 3:
                    self.default = Queen(color)
                if self.column == 4:
                    self.default = King(color)
...
class Board:
    ...
    def __getitem__(self, tup):
        return self.game_board[tup[0]][tup[1]].piece
    ...
    def getMove(self, move: str) -> Move:
        ...
        if matches is None:
            return None  # cannot determine move_input
